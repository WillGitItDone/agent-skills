...
5. **Pre-publish diff summary.** If the skill already exists in the repo, show the
   user what changed before asking for confirmation:
   ```bash
   diff -rq --exclude='.DS_Store' --exclude='local.md' \
     ~/.copilot/skills/{name}/ \
     ~/.copilot/skill-cache/agent-skills/skills/{name}/ 2>&1
   ```
   Then show a condensed content diff of changed files (pipe through `head -80` to
   keep it manageable). Present a summary table:

   > **Changes to publish for {name}:**
   >
   > | File | Status |
   > |------|--------|
   > | SKILL.md | Modified — [brief description of key changes] |
   >
   > Local version: {local_version} → Repo version: {repo_version}

   Ask the user to confirm before proceeding.

6. **Version bump check.** Compare the local `version:` against the repo's `version:`.
   If content has changed but the version is the same (or lower), **prompt the user
   to bump the version** before publishing. Suggest:
   - **Patch** (x.y.Z) for small fixes, typo corrections, clarifications
   - **Minor** (x.Y.0) for new features, added sections, new capabilities
   - **Major** (X.0.0) for breaking changes, major rewrites, workflow changes

   Do not proceed until the version is bumped (update the local SKILL.md, not just
   the repo copy).

7. Check if a skill with the same name already exists in the repo:
   - If yes and user confirmed in step 5, proceed.
   - If no (new skill), ask: "This is a new skill for the repo. Ready to publish?"

8. Copy skill files to repo cache, **excluding local.md**:
   ```bash
...
9. **Regenerate the README** (see README Generation section below).
...
12. **Create a pull request.** If the push succeeds, create a PR using the GitHub CLI:
    ```bash
    cd ~/.copilot/skill-cache/agent-skills
    gh pr create \
      --base main \
      --head skill/{name} \
      --title ":art: Update {name} skill to v{version}." \
      --body "<auto-generated summary of changes>"
    ```
    For the PR body, include:
    - The version change (e.g., "1.1.0 → 2.0.0")
    - A bullet list of key changes (derived from the diff summary in step 5)
    - Note any new dependencies added to `requires`

    For new skills, use title: `:art: Add {name} skill v{version}.`

    If `gh` is not available or the PR creation fails, fall back to giving the user
    the compare URL:
    > ⚠️ Could not auto-create PR. Create one manually:
    > https://github.com/WillGitItDone/agent-skills/compare/skill/{name}?expand=1

13. If the push fails (e.g., no remote configured yet), tell the user:
...
14. **Update the workspace skill table** (see Workspace Sync section below).
...
2. **If it does not exist**, create it with the template:
   ...
   # Bitbucket (API token for curl-based operations)
...
search for literal matches in the skill files. **Skip variables whose names end in
`_URL`, `_USERNAME`, or `_EMAIL`** — these are public identifiers, not secrets, and
matching them causes false positives (e.g., the Jira base URL appearing in examples).

```bash
# Extract values from credentials.env (skip comments, empty lines, and non-secret vars)
grep -E '^export ' ~/.copilot/credentials.env | while IFS='=' read -r key value; do
  varname=$(echo "$key" | sed 's/export //')
  # Skip non-secret variables (URLs, usernames, emails)
  case "$varname" in
    *_URL|*_USERNAME|*_EMAIL|*_USER|*_HOST|*_DOMAIN) continue ;;
  esac
  # Strip quotes from value
  clean_value=$(echo "$value" | sed 's/^["'"'"']//;s/["'"'"']$//')
  if [ ${#clean_value} -ge 8 ]; then
    # Search for the literal value in skill files
    result=$(grep -rl "$clean_value" ~/.copilot/skills/{name}/ 2>/dev/null)
    if [ -n "$result" ]; then
      echo "⚠️ $varname value found in: $result"
    fi
  fi
done
```
...
Use Python to reliably parse YAML frontmatter and generate the README. This avoids
fragile bash-based YAML parsing that breaks on multiline `description` fields and
nested `requires` blocks.

Run this script from the repo cache root (`~/.copilot/skill-cache/agent-skills`):
...
for name in sorted(os.listdir(skills_dir)):
    ...
    for f in ["SKILL.md", "skill.md"]:
        if os.path.exists(os.path.join(skill_path, f)):
            skill_file = os.path.join(skill_path, f)
            break
    if not skill_file:
        continue

    with open(skill_file, "r") as fh:
        content = fh.read()
...
# Build README from the template below
lines = []
lines.append("# Agent Skills\n")
lines.append("Shared skill library for Copilot CLI users at Engrain.\n")
...
for s in skills:
    ...
    lines.append(f"| [{s['name']}](skills/{s['name']}/) | {desc} | {s['version']} |")
...
for s in skills:
    ...
    if req:
        ...
        if req.get("env"):
            lines.append(f"- Environment variables: {', '.join(req['env'])}")
        if req.get("bins"):
            lines.append(f"- CLI tools: {', '.join(req['bins'])}")
        if req.get("tools"):
            lines.append(f"- MCP tools: {', '.join(req['tools'])}")
    lines.append("")

# Append the static footer (Quick Start, Credential Management, Publishing)
...
The `STATIC_FOOTER` is the content from "## Quick Start" through the end of the
README template (see below). Inline it as a Python string in the script.

**Static footer content** (append verbatim after the dynamic skills sections):
...
Write the generated content to `~/.copilot/skill-cache/agent-skills/README.md`.
